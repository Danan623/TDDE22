...
public class AVLTreeNode<T extends Comparable<T>> {
	...
	public AVLTreeNode<T> insert(T data) {
		...
		return balanceTree(nodeHeight(right) - nodeHeight(left), this); // re-balance tree if balancefactor = +2 or -2
	}
	 ...
	 * @return node (address)
	 * */
	private AVLTreeNode<T> balanceTree(int balance, AVLTreeNode<T> node) {
		calculateHeight(node); // calculate height from each node. ex. root to last node or between nodes
		if (balance == -2) { // leftHeavy
			if (nodeHeight(left.left) - nodeHeight(left.right) > 0) { // left->left (-) left->right(empty) = 0 - (-1) =
																		// 1
				return left_left(node);
			} else {// left->left(empty) (-) left->right = -1 - (0) = -1
				return left_right(node);
			}

		} else if (balance == 2) {// rightHeavy
			if (nodeHeight(right.right) - nodeHeight(right.left) > 0) {// right->right (-) right->left(empty) = 0 - (-1)
																		// = 1
				return right_right(node);
			} else {// right->right(empty) (-) right->left = -1 - (0) = -1
				return right_left(node);
			}
		}

		return node; // if already balanced
	}

	private AVLTreeNode<T> right_left(AVLTreeNode<T> node) {
		// TODO Auto-generated method stub
		return doubleRotationWithRightChild(node);
	}

	private AVLTreeNode<T> right_right(AVLTreeNode<T> node) {
		// TODO Auto-generated method stub
		return singleRotationWithRightChild(node);
	}

	private AVLTreeNode<T> left_right(AVLTreeNode<T> node) {
		// TODO Auto-generated method stub
		return doubleRotationWithLeftChild(node);
	}

	private AVLTreeNode<T> left_left(AVLTreeNode<T> node) {
		// TODO Auto-generated method stub
	...
	public AVLTreeNode<T> remove(T key) {
		...
		return balanceTree(nodeHeight(right) - nodeHeight(left), this); // rebalance tree
