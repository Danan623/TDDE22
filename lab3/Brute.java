...
public class Brute {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		points = getPoints(input); // skickar in hela filen här "startar här"
									// ---------------------------------------------------------
		render(frame, points);

		// Sorting points by natural order. Makes finding end points of line segments
		// easy.
		Collections.sort(points, new NaturalOrderComparator()); // sorterar i stigande ordning från vänster

		long start = System.currentTimeMillis();

		// Iterate over and paint all line segments of 4 or more points.
		int count = points.size();
		for (int i = 0; i < count - 3; ++i) { // i = 0
			for (int j = i + 1; j < count - 2; ++j) { // j = 1, j = 2 (= k för 2:a iteration), j = 3 ... kollar samma
														// punkter igen
				for (int k = j + 1; k < count - 1; ++k) { // k = 2, k = 3, k = 4 ...
