...
public class MinPQ<Key_Class> {
	...
	public void insert(Key_Class x) {
		...
		if (size() == pq.length - 1) {// static -> dynamic array
			resize(2 * count);
		}
		count++;
		pq[count] = x;
		swim(count); // "swim" up in the array when insert. To create a minHeap.
		orderOK = true;
	}
	...
	public void toss(Key_Class x) {
		orderOK = false; // Double the size of the queue as needed
